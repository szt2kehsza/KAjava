public class AVariableModifications {
    public static void main(String[] args) {
        int a = 3;
        // increase the value of variable "a" by 10 and print the new value
        System.out.print("1.: ");
        System.out.println(a+10);

        int b = 100;
        // decrease the value of b by 7 and print the new value
        System.out.print("2.: ");
        System.out.println(b-7);


        int c = 44;
        // double the value of c and print the new value
        System.out.print("3.: ");
        System.out.println(c*2);

        int d = 125;
        // divide the value of d by 5 and print the new value
        System.out.print("4.: ");
        System.out.println(d/5);

        int e = 8;
        // cube the value of e and print the new value
        System.out.print("5.: ");
        System.out.println(Math.pow(8, 2));

        int f1 = 123;
        int f2 = 345;
        // determine if f1 is greater than f2 (print as a boolean)
        System.out.print("6.: ");
        boolean seged = false;
        if (f1>f2){
            seged = true;
            System.out.println(seged);
        }else{
            System.out.println(seged);
        }


        int g1 = 350;
        int g2 = 200;
        // determine if double the value of g2 is greater
        // than g1 (print as a boolean)
        System.out.print("7.: ");
        boolean seged1 = true;
        if (g2*2 > g1){
            seged1 = true;
        }else {
            seged1 = false;
        }
        System.out.println(seged1);

        int h = 135798745;
        // determine if h has 11 as a divisor (print as a boolean)
        System.out.print("8.: ");
        boolean seged2 = false;

        if (h%11==0){
            seged2 = true;
            System.out.println(seged2);
        }else{
            System.out.println(seged2);
        }



        int i1 = 10;
        int i2 = 3;
        // determine if i1 is greater than i2 squared and smaller than i2 cubed
        // (print as a boolean)
        System.out.print("9.: ");
        boolean seged3 = false;

        if (i1>Math.sqrt(i2) && Math.sqrt(i2)<Math.pow(i2, 2)){
            seged3 = true;
            System.out.println(seged3);
        }else{
            System.out.println(seged3);
        }


        int j = 1521;
        // determine if j is divisible by 3 or 5 (print as a boolean)
        System.out.print("10.: ");
        boolean seged4 = false;

        if (j %3 == 0 || j%5==0){
            seged4 = true;
            System.out.println(seged4);
        }else{
            System.out.println(seged4);
        }

    }
}
